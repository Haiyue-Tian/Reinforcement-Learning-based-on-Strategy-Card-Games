...
class PreLayer(nn.Module):
    def __init__(self):
        ...
        # self.fc1 = nn.Linear(551, 1024)
        self.fc2 = nn.Linear(551, 3136)
        self.conv = nn.Conv2d(in_channels=1, out_channels=24, kernel_size=3, stride=1, padding=1)
        self.bn = nn.BatchNorm2d(24)
        self.hs = HardSwish(inplace=True)
...
class MobileNetV3(nn.Module):
    def __init__(self, num_classes=1, type='small'):
        ...
        if type=='large':
            ...
            self.small_bottleneck = nn.Sequential(
                SEInvertedBottleneck(in_channels=24, mid_channels=72, out_channels=24, kernel_size=3, stride=2, activate='relu', use_se=False),
