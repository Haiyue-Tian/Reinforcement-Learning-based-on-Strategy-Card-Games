...
class DMCAgent:
    def __init__(self,
        ...
        self.device = torch.device('cuda:'+str(device_ids[0])) if type(device_ids[0])!=str else torch.device(device_ids[0])
    ...
    def predict(self, state):
        ...
        stateAction = np.array([])
        for i in range(len(action_values)):
            s = np.hstack((state['obs'], action_values[i]))
            s.reshape((self.state_shape[0]+self.action_shape[0]))
            stateAction = s if stateAction.shape[0] == 0 else np.vstack((stateAction, s))

        num = np.int(np.ceil(len(action_values)/self.max_split_size))
        for n in range(num):
            if n != num-1:
                q_values[n * self.max_split_size:(n + 1) * self.max_split_size] = self.net.forward(
                    torch.from_numpy(stateAction[n*self.max_split_size:(n+1)*self.max_split_size]
                    ).to(self.device).to(torch.float32)).cpu().detach().numpy().reshape(self.max_split_size)
            else:
                q_values[n * self.max_split_size:] = self.net.forward(torch.from_numpy(
                    stateAction[n * self.max_split_size:]).to(self.device).to(torch.float32)).cpu().detach().numpy().reshape(
                    len(q_values[n * self.max_split_size:]))
    ...
    def gpu_name(self, data_dir, gpu_num, coords, other_coords):
    ...
    def learn(self, memory, memory_size, batch_size, log_path, coords, other_coords,
              pauses, epoch, gpu, collect_data_num, data_dir):
        length = len(coords)
        writer = SummaryWriter(log_path)
        global_step = 0
        memory_idx = 0
        count_flag = 0
        count_name, data_name = self.gpu_name(data_dir, gpu, length, other_coords)
        try:
        ...
        while True:
            ...
            if total_memory_num>=collect_data_num and length<total_memory_num and train_flag:
                # 读取数据
                data_flag = 1
                ...
                with open(data_dir+'memory.pickle', 'wb') as f:
                    memory_tmp = {'memory':memory, 'memory_idx': memory_idx}
                    pickle.dump(memory_tmp, f)
                # 暂停其他进程
                for i in range(length):
                    pid = coords[i].pid
                    exec(pauses[i] + '= psutil.Process(pid)')
                    exec(pauses[i] + '.suspend()')
                # 这一次的循环轮数
